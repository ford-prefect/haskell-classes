...
instance Foldable (TreeMap k) where
  ...
  foldMap f (Node k v l r) = foldMap f l <> f v <> foldMap f r

  foldr f init EmptyNode = init
  foldr f init (Node k v l r) = foldr f (f v (foldr f init r)) l
...
prop_nonMemberGetIsNothing l notk = isNothing (get t notk) where t = fromList [(k, v) | (k, v) <- l, k /= notk]
